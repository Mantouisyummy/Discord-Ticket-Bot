...
import disnake
from disnake import (ApplicationCommandInteraction, Colour, Member, Option,
                     OptionType)
...
class View(disnake.ui.View):
    ...
    @disnake.ui.button(
        label="點我創建", style=disnake.ButtonStyle.green, custom_id="create_ticket"
    )
    async def create_ticket(
        ...
        if interaction.author.id == author:
            ...
            try:
                self.ticket_channel = disnake.utils.get(
                    interaction.guild.text_channels, id=self.ticket_channel.id
                )
            except AttributeError:
                pass
            if self.ticket_channel is not None:  # 如果已經創建過了,即跳出此訊息
                failed_embed = disnake.Embed(
                    title="<:x_mark:1033955039615664199> | 你已經有這個頻道了!",
                    description=f"你的頻道在 {self.ticket_channel.mention}",
                    colour=disnake.Colour.red(),
                )
                await interaction.response.send_message(
                    embed=failed_embed, ephemeral=True
                )
            else:
                loading_embed = disnake.Embed(
                    title="<a:Loading:1059806500241027157> | 正在創建中...",
                ...
                self.ticket_channel = await interaction.guild.create_text_channel(
                    f"客服單-{interaction.user.name}",
                ...
                channel_embed = disnake.Embed(
                    title=f"{interaction.user.name} 的客服單",
                    description="請等待人員處理您的問題",
                    colour=Colour.random(),
                )
                view = TicketView()
                member = interaction.user  # 用戶
                message = await self.ticket_channel.send(
...
class TicketView(disnake.ui.View):  # 客
    ...
    @disnake.ui.button(
        label="鎖定客服單", style=disnake.ButtonStyle.blurple, custom_id="lock_ticket"
    )
    async def lock_ticket(
        ...
        if (
            ...
            if self.lock == True:  # 偵測是否已經鎖定
                try:
                    conversation_record = await self.channel.history(
                        limit=None
                    ).flatten()  # 讀取頻道的歷史訊息
                    text = ""  # 將字串變為空
                except NameError:  # 如果遇到錯誤
                    ticket_channel = disnake.utils.get(
                        interaction.guild.text_channels, id=self.channel.id
                    )  # 抓取客服單的頻道
                    conversation_record = await ticket_channel.history(
                        limit=None
                    ).flatten()  # 讀取頻道的歷史訊息
                    text = ""  # 將字串變為空
                ...
                await interaction.response.send_message(
                    f"客服單已被 {interaction.user.name} 鎖定!"
                )
                await interaction.followup.send(
                    f"本次對話紀錄檔案:", file=disnake.File(f"chat.txt")
                )  # 傳送一則帶有對話紀錄檔案的訊息
            ...
            await interaction.response.send_message(
                "你無法鎖定客服單! 請聯繫管理人員以鎖定此克服單", ephemeral=True
            )

    @disnake.ui.button(
        label="刪除客服單", style=disnake.ButtonStyle.red, custom_id="del_ticket"
    )
    async def delete_ticket(
        ...
        if (
            ...
            loading_embed = disnake.Embed(
                title="<a:Loading:1059806500241027157> | 正在刪除...",
                colour=disnake.Colour.light_grey(),
            )
            ...
            await interaction.response.send_message(
                "你無法刪除客服單! 請聯繫管理人員以刪除此客服單", ephemeral=True
            )
...
class ticket(commands.Cog):
    ...
    @ticket.sub_command(
        ...
        options=[
            Option(name="text", description="設定您想給大家知道用途的介紹文字 (留空自動生成)", required=False)
        ],
    )
    ...
    async def create(self, interaction: ApplicationCommandInteraction, text: str):
        if text != None:  # 偵測是否留空
            embed = disnake.Embed(
                title="開啟客服單的系統", description=f"{text}", colour=disnake.Colour.orange()
            )
        else:
            embed = disnake.Embed(
                title="開啟客服單的系統",
                description="如果你需要和服務人員聯絡\n請點擊下方按鈕開啟客服單",
    ...
    @ticket.sub_command(
        name="ban",
        description="封禁一位使用者使用ticket系統",
        options=[
            Option(
                name="member",
                description="選擇禁止使用ticket的用戶",
    ...
    @commands.has_permissions(manage_messages=True)
    async def ban(
        self, interaction: ApplicationCommandInteraction, member: Optional[Member]
    ):
        ...
        if member.name not in data:  # 偵測用戶在不在名單中
            ...
            embed = disnake.Embed(
                title="✅ | 成功!",
                description=f"已封鎖了 {member.mention}",
                colour=Colour.green(),
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        else:  # 有則顯示
    ...
    @ticket.sub_command(
        name="unban",
        description="解封一位使用者使用ticket系統",
        options=[
            Option(
                name="member",
                description="選擇解封使用ticket的用戶",
                type=OptionType.user,
                required=True,
            )
        ],
    ...
    async def unban(
        ...
        if member.name in data:
            ...
            embed = disnake.Embed(
                title="✅ | 成功!",
                description=f"已解封了 {member.mention}",
                colour=Colour.green(),
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        else:
